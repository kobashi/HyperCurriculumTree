...
function showcaseMarkup() {
  return `<div class="showcase" aria-label="4コースの担当教員アトラス">
    <div class="showcase-heading"><p class="showcase-kicker">FACULTY ATLAS / 2026</p><p>マウス移動・ドラッグ・スワイプで教員をたどる。写真を選ぶと担当科目へ。</p></div>
    <div class="showcase-stage" tabindex="0" aria-label="4コースの担当教員。マウスを動かすか、ドラッグ・スワイプで教員を選べます">${showcasePeople.map(({ course, group, targetId }, index) => {
      ...
      return `<a class="showcase-person${index === 0 ? " is-current" : ""}" data-course="${course.id}" href="#${targetId}" aria-label="${escapeHtml(course.name)}コース・${name}の担当科目へ" tabindex="${index === 0 ? "0" : "-1"}"${index === 0 ? ' aria-current="true"' : ""}><span class="showcase-portrait">${avatar}</span><span class="showcase-person-name">${name}</span><span class="showcase-person-course">${escapeHtml(course.name)}</span></a>`;
    }).join("\n")}</div>
    ...
    <div class="showcase-course-legend" aria-label="教員の並び順">${courses.map((course) => `<span data-course="${course.id}"><b>${course.number}</b>${course.name}</span>`).join("")}</div>
  </div>`;
}
  ...
  <div class="page-shell">
    <header class="masthead">
      ...
      <div class="masthead-main">
        ...
        <p class="intro">4つのコースを横断して、担当者をたどる。</p>
      </div>
      ${showcaseMarkup()}
    </header>
    ...
    <footer class="source-note">
      ...
      <div class="source-body">
        <p class="source-summary">4つのコースで学ぶ科目と、2026年度の時間割に記載された担当者の一覧です。写真は大学公式サイトに掲載されている場合のみ表示しています。</p>
        <div class="source-facts"><span><strong>04</strong> コース</span><span><strong>${subjects.length}</strong> 科目掲載</span><span><strong>${uniqueTeachers.size}</strong> 担当者</span><span><strong>${picturedTeachers.size}</strong> 写真あり</span></div>
